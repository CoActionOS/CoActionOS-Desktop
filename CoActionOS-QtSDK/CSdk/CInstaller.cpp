...
void CInstaller::emitUpdated(){
    emit projectUpdated(ui->workspaceLine->lineEdit()->text(),
                        ui->projectNameComboBox->currentText(),
                        ui->configurationComboBox->currentText());
}

void CInstaller::savePreferences(){
    CSettings s(CSettings::userScope());
    s.setKey(settingsPrefix + "WORKSPACE", ui->workspaceLine->lineEdit()->text());
    s.setKey(settingsPrefix + "PROJECT", ui->projectNameComboBox->currentText());
    s.setKey(settingsPrefix + "CONFIGURATION", ui->configurationComboBox->currentText());
}


void CInstaller::loadPreferences(){
    CSettings s(CSettings::userScope());

    QString workspace = s.getStringKey(settingsPrefix + "WORKSPACE");
    QString project = s.getStringKey(settingsPrefix + "PROJECT");
    QString conf = s.getStringKey(settingsPrefix + "CONFIGURATION");

    ui->workspaceLine->lineEdit()->setText( workspace );
    ui->projectNameComboBox->setCurrentText( project );
    ui->configurationComboBox->setCurrentText( conf );

    qDebug("Load installer %s %s %s",
...
void CInstaller::selectProject(QString project){
    int i;
    i = ui->projectNameComboBox->findText(project);
    if( i < 0 ){
        return;
    }

    ui->projectNameComboBox->setCurrentIndex(i);
}


void CInstaller::selectConfiguration(QString conf){
    int i;
    i = ui->configurationComboBox->findText(conf);
    if( i < 0 ){
        return;
    }

    ui->configurationComboBox->setCurrentIndex(i);
}
...
void CInstaller::showLabels(bool v){
    ui->workspaceLabel->setVisible(v);
    ui->projectNameLabel->setVisible(v);
    ui->configurationLabel->setVisible(v);
