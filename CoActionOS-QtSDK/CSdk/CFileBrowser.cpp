...
void CFileBrowser::saveFileToDeviceSelected(QString filename){
  ...
  targetDir = ui->tree->currentItem()->text(1);
  ...
  if( (targetDir == "/app") ||
      ...
      qDebug("Copy file");
      err = link()->cp(filename.toStdString(), target.toStdString(), 0777, true, updateProgress);
    }
  if ( err < 0 ){
      qDebug("Err is %d", err);
  ...
  CNotify::updateStatus("Downloaded to:  " + target);
}
